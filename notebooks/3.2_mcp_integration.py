...
_whl = f"/Workspace/Users/{_username}/.bundle/dev/course-code-hub/artifacts/.internal/arxiv_curator-0.8.0-py3-none-any.whl"
...
try:
    ...
    vs_tools = []
    logger.warning(f"⚠️ MCP endpoint not available: {type(e).__name__}. Feature may not be enabled on this workspace.")
...
try:
    ...
    logger.warning(f"⚠️ MCP call_tool failed: {type(e).__name__}. Skipping.")
...
if hasattr(cfg, 'genie_space_id') and cfg.genie_space_id:
    ...
    try:
        ...
        logger.warning(f"⚠️ Genie MCP not available: {type(e).__name__}")
else:
...
# Create tools
mcp_tools = create_mcp_tools(w, mcp_urls)
